...
!
	...
	real td2 ! calibration parameter for direct shortwave radiation transmitted through ice (-)
	real ts ! calibration parameter for diffuse shortwave radiation transmitted through ice (-)
	real zandeg ! zenith angle of the Sun (degree, parameter)
	real candeg ! zenith angle from the CH center to edge (degree, parameter)
...
!
	call para(metfile,outfile,ylat,alc,ali,atp,ddc,cd,srf,chi,nd,nss,dcay,td,ts,zandeg,candeg)
...
!
	  ...
	  if(zandeg.ge.0)then ! replace solar zenith angle
         zan = zandeg*pi/180
        endif
...
! determine zenith angle of CH edge and sky view
	  ...
	  if(candeg.ge.0)then ! replace solar zenith angle
         can = candeg*pi/180
        endif
! determine direct solar radiation 
	  if(zan.lt.can)then ! direct solar radiation from sky
	   ...
	   if(zan.lt.(0.5*pi))then
	    ...
	    call transmit(cd/cos(zan),tid,dif,td2,kid) ! transmittance for direct component
	   else
...
!  read variable parameters
	subroutine para(metfile,outfile,ylat,alc,ali,atp,ddc,cd,srf,chi,nd,nss,dcay,td,ts,zandeg,candeg)
	implicit none
	integer nd,nss
	real ylat,alc,ali,atp,ddc,cd,srf,chi,dcay,td,ts,zandeg,candeg
	...
	read(10,*) cdum,ts ! tuning parameter for transmittance diffuse SR (relative to direct SR)
	read(10,*) cdum,zandeg ! solar zenith angle
	read(10,*) cdum,candeg ! CH zenith angle
	close(10)
	return
	endsubroutine
!!
